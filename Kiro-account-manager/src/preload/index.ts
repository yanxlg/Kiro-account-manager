...
const api = {
  ...
  mcpGetConfig: (): Promise<unknown> => {
    return ipcRenderer.invoke('mcp:get-config')
  },

  mcpSaveConfig: (patch: unknown): Promise<unknown> => {
    return ipcRenderer.invoke('mcp:save-config', patch)
  },

  mcpSaveServer: (input: unknown): Promise<unknown> => {
    return ipcRenderer.invoke('mcp:save-server', input)
  },

  mcpDeleteServer: (input: { name: string }): Promise<unknown> => {
    return ipcRenderer.invoke('mcp:delete-server', input)
  },

  mcpImportFromAgents: (input: { agents?: string[]; overwrite?: boolean }): Promise<unknown> => {
    return ipcRenderer.invoke('mcp:import-from-agents', input)
  },
  ...
  onMcpConfigChanged: (callback: () => void): (() => void) => {
    ...
    return () => {
      ipcRenderer.removeListener('mcp:config-changed', handler)
    }
  },
  ...
  onSkillsUpdateStatusChanged: (
    ...
    const handler = (
      _event: Electron.IpcRendererEvent,
      data: { agent: string; skillName: string; status: string; reason?: string }
    ): void => {
      callback(data)
    }
  ...
  onSkillsCheckProgress: (
    ...
    const handler = (
      _event: Electron.IpcRendererEvent,
      data: { agent: string; skillName: string; checking: boolean }
    ): void => {
      callback(data)
    }
