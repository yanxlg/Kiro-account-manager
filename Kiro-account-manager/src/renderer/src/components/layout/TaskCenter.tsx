...
function TaskCenterDrawer({ open, onClose }: TaskCenterDrawerProps): React.ReactNode {
  ...
  const cancelAllActive = (): void => {
    ...
    if (!confirm(`取消正在进行的 ${active.length} 个任务？`)) return
      ...
      {open && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 bottom-0 z-50 w-[420px] bg-background border-l shadow-2xl flex flex-col"
          >
            {/* 头部 */}
              ...
              <div className="flex items-center gap-1">
                {activeCount > 0 && (
                  <Button
                    ...
                    title={`取消所有 ${activeCount} 个正在进行的任务`}
                  >
...
function TaskRow({ task, onCancel, onRemove }: TaskRowProps): React.ReactNode {
  ...
  const statusBadge = (() => {
    switch (task.status) {
      case 'running': return <Badge className="bg-primary text-primary-foreground">运行中</Badge>
      case 'paused': return <Badge variant="outline" className="text-amber-600 border-amber-200">已暂停</Badge>
      case 'success': return <Badge variant="outline" className="text-green-600 border-green-200">已完成</Badge>
      case 'failed': return <Badge variant="outline" className="text-red-600 border-red-200">失败</Badge>
